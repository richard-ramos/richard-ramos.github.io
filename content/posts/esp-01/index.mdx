...
The process is similar to connecting to an Arduino (the same connections), however these adapters might not output the current required by the ESP-01. You might end up requiring an external power source to provide the 3.3v. Be sure to read the datasheet of your adapter to know how much current it can provide. The one i'm using outputs 500mA which is enough for it.
...
There are some cheap USB - UART programmers that can be turned into programmers. In this video the process is meticulously explained: https://www.youtube.com/watch?v=6uaIWZCRSz8 . It's an interesting alternative and one that I'll be trying in the future
